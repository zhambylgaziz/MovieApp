...
class RecommendedMoviesAdapter(private val movies: ArrayList<Movie> = ArrayList()) : RecyclerView.Adapter<RecommendedMoviesAdapter.ViewHolder>(){

    private lateinit var movieClickListener: MovieClickListener

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecommendedMoviesAdapter.ViewHolder
            = ViewHolder(
    ...
    class ViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        fun bindMovie(movie: Movie) {
            with(view) {
                Picasso.get().load(BuildConfig.URL_POSTER + movie.poster).into(poster)
